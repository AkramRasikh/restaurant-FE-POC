...
Come back to these notes:

Build page

1. gatsby-node.js
2. extracted to different files
   1. Can also extract into a plugin
3. source node is called
   1. Where the REST API data is called
      1. Restaurant info
      2. configuration data!!!
   2. Creating nodes
      1. createContentDigest
         1. contentDigest -> is an Id reference for caching comparison
            reference?
         2. content -> JSON object
         3. type -> the reference we pass in the query that we will also use for
            page references
            1. (I.E restaurantsType.restaurantId)
         4. createNodeId -> must be unique if creating a multiple of nodes from
            the same type -> think of it like keys in react

———————————————————

Initially with explicitly defined routes we had to map over them and the call
‘createPage’

We had messy conditionals but now we can directly reference config in the
on-create-Page.js file

now we reference our config directly by the `getNodesByType(‘configType’)`

and we can deletePages accordingly

Why delete instead of conditionally create the page??? -> this I don’t know yet

It doesn’t really matter anyway because they don’t actually get BUILT

———————————————————

Need to figure out: force routing at home page level Need to figure out: how not
to build all pages for all restaurants
